...
public class WeatherGenerator {
                                     ...
                                     double[][] drywet, double[][] wetwet)
    {
        ...
        for(int k=0;k<wetwet.length;k++)
        {
            if(wetwet[k][0]==longitude&& wetwet[k][1]==latitude)
            {
                int l = 2;
                while( l < 14)
                {
                    wetwetProbability[l-2] = wetwet[k][l];
                    l++;
                }
            }
        }
    ...
    public static int[] oneMonthForecast(int numberOfLocations, int month, double longitude, double latitude ){
            ...
            double drywetProbabilityy=drywetProbability[month];
            double wetwetprobabilityy= wetwetProbability[month];
            return forecastGenerator(drywetProbabilityy, wetwetprobabilityy, numofDays);
    ...
    public static int bestWeekToTravel(int[] forecast){
        ...
        for (int i = 0; i < forecast.length; i++) {
            for (int j = i;  j < forecast.length&&j < i + 7 ; j++) {
